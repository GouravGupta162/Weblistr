<div class="main-content">
 ...
 <a class="btn btn-xs btn-danger pull-right" href="admin/user/add" >Add New User</a>
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <table id="dynamic-table" class="table table-striped table-bordered table-hover comp_user">
                                <thead>
                                    <tr>
                                ...
                                <tbody id='all_rows'>
									...
									if(sizeof($getcmpUser)>0)
									{
                                    foreach ($getcmpUser as $usr) {
                                        $usr_id = $usr['uid']; 
										$cmp_stts = $usr['status'];
...
onclick="usercompanyactive('<?php echo $usr_id ?>','<?php $var = $status; echo $var_is_greater_than_two = ($var == 0 ? '1' : '0'); ?>')" >
...
<script>
function approveassign(usr)
{
	var baseHref = document.getElementsByTagName('base')[0].href;
	$.ajax({
		type: 'post',
		url: baseHref+'/admin/user/approveassign',
		data: { 'usr_id':usr },
		success: 
			function (response) {
				if(response == '1')
...
$(window).scroll(function(){
	...
	function loadmoreReviews()
	{
		...
		$.ajax({
			type: 'post',
			url: baseHref+'/admin/company/getcmpUseronscroll',
			data: { getresult:val },
			success: 
				function (response) {
...
function usercompanyactive(id,stts)
{
	 ShowProgress();
    ...
    function postForm(usr_id, stts)
    {
        ...
        post("admin/user/companyUserStatusUpdate", params, 'POST');
    }
