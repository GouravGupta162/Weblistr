...
( function( $, window, document, undefined ) {
  ...
  MasterBlaster.prototype.onAdd = function( e ) {
    if( e.type === "click" || ~this.options.triggerKeys.indexOf( e.keyCode || e.which ) ) {
	  ...
	  if(tagName == ""){
		  //alert("please fill tag value");
	  }
	  else if( this.isValid( tagName ) ) {
        this.$container.removeClass( "mb-error" );
		if($('#hidden_tag').val()){
			$('#hidden_tag').val($('#hidden_tag').val()+","+tagName);
		}
		else {
			$('#hidden_tag').val(tagName);
		}
        this.push( tagName );
        this.$input.val( "" );
      }
  ...
  MasterBlaster.prototype.onRemove = function( e ) {
    e.preventDefault( );
	//console.log($( e.target ).parents( ".mb-tag" ).attr( "data-tag" ));
	var removetext = $( e.target ).parents( ".mb-tag" ).attr( "data-tag" );
	//var split = $('#hidden_tag').val().split(',');
	
	if($('#hidden_tag').val().indexOf(',') != -1){
		var remains = $('#hidden_tag').val().replace(removetext+",","");
		$('#hidden_tag').val(remains);
	}
	else{
		$('#hidden_tag').val('');
	}
